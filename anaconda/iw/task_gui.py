...
import isys
import shutil
#import ctl_checkbutton
from iw.ctl_checkbutton import *
...
class TaskWindow(InstallWindow):
    def getNext(self):
        CONF_FILE = "/mnt/sysimage/suncloud.conf"
        fb = open(CONF_FILE, "w")
        fb.writelines("LOCALIP="+self.entryLocalIp.get_text()+"\n")
        fb.writelines("LOCALNETMASK="+self.entryLocalNetmask.get_text()+"\n")
        fb.writelines("LOCALGATEWAY="+self.entryLocalGateway.get_text()+"\n")

        #flag1 kvm; flag2 cdms
        flag1 = self.xml.get_widget("checkbutton1").get_active()
        flag2 = self.xml.get_widget("checkbutton2").get_active()
        
        if flag1 or flag2:
            if flag2:
                if len(self.entryServerIp.get_text()) == 0 or len(self.entryServerPasswd.get_text()) == 0:
                    self.dispatch.skipStep("basepkgsel", skip = 0)
                fb.writelines("FLAGCDMS=ENABLE"+"\n")
                fb.writelines("SERVERIP="+self.entryServerIp.get_text()+"\n")
                fb.writelines("SERVERPASSWD="+self.entryServerPasswd.get_text()+"\n")
            if flag1:
                if len(self.entryCdms.get_text()) == 0:
                    self.dispatch.skipStep("basepkgsel", skip = 0)
                fb.writelines("FLAGKVM=ENABLE"+"\n")
                fb.writelines("CDMSIP="+self.entryCdms.get_text()+"\n")

        interface = self.combobox.get_active_text() 
        fb.writelines("INTERFACE="+interface+"\n")
        fb.close()
    ...
    def kvm_callback(self, widget):
        if widget.get_active():
            setKvmFlags(1)
            self.labelCdms.set_sensitive(True)
            self.entryCdms.set_sensitive(True)
        else:
            setKvmFlags(0)
            self.labelCdms.set_sensitive(False)
            self.entryCdms.set_sensitive(False)
            self.entryCdms.set_text("")

    def cdms_callback(self, widget):
        if widget.get_active():
            setCdmsFlags(1)
            self.labelServerIp.set_sensitive(True)
            self.entryServerIp.set_sensitive(True)
            self.labelServerPasswd.set_sensitive(True)
            self.entryServerPasswd.set_sensitive(True)
        else:
            setCdmsFlags(0)
            ...
            self.entryServerIp.set_text("")
            self.entryServerPasswd.set_text("")
